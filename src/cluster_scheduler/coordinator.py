...
# -*- coding: utf-8 -*-
"""
基于协同诱饵的基线智能体（支持 K=2 协同）。

核心策略：
  * 分区搜索：每架无人机扫描一个经度子区域。
  * EMA + 回归速度识别假目标：区分运动真目标（5‑12 米/秒）与静止假目标。
  * K=2 协同：通过 CooperativeCoordinator 实现：
      - 无人机A确认真目标 → 广播 R:… 召唤队友。
      - 无人机B收到 R:… → 飞向 SLOT_1 瞄准点（与 SLOT_0 距离 >200 米）。
      - 两机同时检测到同一目标 → 各自累计 dwell 时间。
      - 双方 dwell 均 ≥20 秒 → 目标被环境摧毁。
      - 目标摧毁后广播 C:… → 两机切换至下一个目标。
  * 贪婪目标选择：每架无人机独立选择最近的活动目标，优先选择队友已召唤的目标。
"""
