...
> **诱饵判别**：诱饵静止、真目标移动——建议采用多帧位置变化做运动学一致性判别。渲染画面的车型/颜色差异在代码层不可读。
...
| `rejected_range` | int | 因超距离（约 1000 m）被拒数 |
...
仅赛题二、三可用；赛题一无队友。受引擎四级限制：字节不超过 50、速率 4 Hz 滑窗、距离约 1000 m、干扰区。
