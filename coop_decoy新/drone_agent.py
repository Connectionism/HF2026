...
目录结构:
    ...
    ├─ vision_detect/main.py          ← EMATracker + DecoyClassifier（EMA滤波 + 诱饵判别）
...
from .search_track.vision_detect.main import EMATracker, DecoyClassifier
...
class DroneAgent(CoopAgent):
    ...
    # ── 图像传感器回调（sensor_skip=false，SDK 自动运行默认识别器） ──

    def sensor(self, obs: CoopObs, dt: float) -> Optional[List[Command]]:
        """
        图像传感器回调。sensor_skip=false 时 SDK 在 sensor() 返回后
        自动运行默认识别器并填充 obs.self.detection，因此本方法仅用于
        photo 接收统计，不负责检测逻辑。
        """
        try:
            ...
            logger.debug("[uid=%s] sensor() error (non-fatal): %s",
                         self.my_uid, traceback.format_exc())

        # sensor_skip=false 时返回值被忽略，SDK 使用默认识别器结果
        return None
